<template>
  <div class="content">
    <!-- <div v-for="(item, index) in windowListInner" :key="index">{{ item.name }}</div> -->
    ...
    <div class="windowList" ref="windowList" v-if="showWindowList && windowList && windowList.length > 1"
      ...
      <WindowListItem v-for="(item, index) in windowListInner" ref="WindowListItem" :key="index" :item="item" :index="index"
...
export default {
  ...
  methods: {
    ...
    setInitData() {
      ...
      console.log('this.windowListInner', this.windowListInner)
